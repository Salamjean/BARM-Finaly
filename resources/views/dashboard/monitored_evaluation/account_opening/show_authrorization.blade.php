    ...
    <div class="container-fluid">
            ...
            <div class="border-start border-primary border-3 ps-3">
                <nav aria-label="breadcrumb">
                    <div class="d-flex align-items-center">
                        <i class="bx bx-shield-check text-primary fs-4 me-3"></i>
                        <div>
                            <div class="text-muted small">Suivi-Évaluation / Autorisation ouverture de compte / Cohorte</div>
        ...
        <div class="bg-white p-4 rounded-3 shadow-none mb-4">
            <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    <i class="bx bx-check-shield text-success fs-3 me-3"></i>
                    <div>
                        <h5 class="mb-0 text-dark">Autorisations d'ouverture</h5>
                        <small class="text-muted">Validation finale pour l'ouverture des comptes</small>
                    </div>
                </div>
                <div class="d-flex align-items-center gap-3">
                    <a href="{{ route('export.pdf.account_openings', $cohort->id) }}" class="btn btn-danger btn-sm text-white shadow-sm">
                    ...
                    <div class="text-center">
                        ...
                        <small class="text-muted d-block">Autorisés</small>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="input-group">
                                ...
                                <input type="text" id="searchMecano" class="form-control" placeholder="Rechercher par Mecano">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card h-100 adherent-card shadow-sm">
                                <div class="card-body position-relative">
                                    ...
                                    <div class="text-center mb-4">
                                        ...
                                        <span class="badge bg-secondary adherent-mecano">{{ $adherent->user->mecano }}</span>
                                    </div>
                                    ...
                                    <div class="border-top pt-3">
                                        @if ($adherent->phone_number)
                                            <div class="mb-3">
                                                ...
                                                <div class="fw-medium">
                                                    ...
                                                    @if ($adherent->phone_number2), {{ $adherent->phone_number2 }}@endif
                                                    @if ($adherent->phone_number3), {{ $adherent->phone_number3 }}@endif
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        @if ($adherent->partner_technical_id)
                                            <div class="mb-3 p-2 border border-primary rounded">
                                                ...
                                                <div class="fw-medium">{{ $adherent->partnerTechnical->user->username }}</div>
                                            </div>
                                        @endif
                                        ...
                                        @if ($adherent->partner_financial_id)
                                            <div class="mb-3 p-2 border border-secondary rounded">
                                                ...
                                                <div class="fw-medium">{{ $adherent->partnerFinancial->user->username }}</div>
                                            </div>
                                        @endif

                                        @if ($adherent->other_partner_financial)
                                            <div class="mb-3 p-2 border border-secondary rounded">
                                                <div class="d-flex align-items-center mb-1">
                                                    <i class="bx bx-bank text-secondary me-2"></i>
                                                    <small class="text-muted">Source de financement</small>
                                                </div>
                                                <div class="fw-medium">{{ $adherent->other_partner_financial }}</div>
                                            </div>
                                        @endif
                                            ...
                                            <div class="mb-3 p-2 border border-info rounded">
                                                ...
                                                @if (!$adherent->other_partner_financial)
                                                    <div class="small mb-2">
                                                        <strong>Commission :</strong> {{ dateFr($adherent->paAccepted->commission->date) }}
                                                    </div>
                                                @endif
                                                <div class="small mb-2">
                                                ...
                                                <div class="small mb-2">
                                                    <strong>Montant :</strong> {{ amount($adherent->paAccepted->amount) }} {{ DEVICE }}
                                                </div>
                                                @if (!$adherent->other_partner_financial)
                                                    <div class="small">
                                                        <strong>Crédit souhaité :</strong> {{ amount($adherent->paAccepted->credit) }} {{ DEVICE }}
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
                                        ...
                                        @if ($adherent->selfEmploymentMonitoredPayment)
                                            <div class="mb-3 p-2 border border-warning rounded">
                                                <div class="d-flex align-items-center mb-2">
                                                    <i class="bx bx-calendar-check text-warning me-2"></i>
                                                    <small class="text-muted fw-bold">Retrait de fiche</small>
                                                </div>
                                                <div class="small mb-2">
                                        ...
                                        @if ((can('chef-celulle-suivi-evaluation|responsable-suivi-evaluation') || can('partner-financial')) && $adherent->selfEmploymentMonitoredPayment)
                                            <div class="border-top pt-3">
                                                <form class="formSubmit" 
                                                      action="{{ route('monitored-evaluation.account_opening.authorization_approved', $adherent->selfEmploymentMonitoredPayment->id) }}" 
                                                      method="post">
                                                    @method('PUT')
                                                    @csrf
                                                    <div class="mb-3">
                                        ...
                                        <tr class="align-middle">
                                            ...
                                            <td>
                                                <div class="border-start border-info border-3 ps-2 py-1">
                                                    <div class="fw-medium text-info">{{ $adherent->paAccepted->title }}</div>
                                                </div>
                                            </td>
                                            <td>
            ...
            $(document).ready(function() {
                $('#searchMecano').on('keyup', function() {
                    ...
                    $('.adherent-item').filter(function() {
                        $(this).toggle($(this).find('.adherent-mecano').text().toLowerCase().indexOf(value) > -1)
                    });
                });
                ...
                $('.formSubmit').submit(function(e) {
                    ...
                    Swal.fire({
                        title: 'Êtes-vous sûr d\'autoriser l\'ouverture du compte?',
                        icon: 'warning',
                        iconColor: '#E68200',
