        ...
        <div class="bg-white p-4 rounded-3 shadow-none mb-4">
            <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h4 class="mb-0 text-warning">Candidats en attente pour la commission</h4>
                        ...
                        <tbody>
                            @foreach ($candidats as $index => $adherent)
                                <tr class="align-middle">
                                    ...
                                    <td>
                                        <div class="border-start border-info border-3 ps-2 py-1">
                                            <div class="fw-bold text-info">{{ $adherent->focal_point_area }}</div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            @if (can('chef-cellule-formation-et-insertion|conseiller-auto-emploi'))
                                                <button type="button" class="btn btn-outline-warning btn-sm open-modal-btn"
                                                    ...
                                                    data-focal-point="{{ $adherent->focal_point_area ?? '' }}"
                                                    title="Ajouter un point focal">
                                                    <i class="bx bx-map-pin me-1"></i>
                                                    Point focal
                                                </button>
                                                @php
                                                    $last = $adherent->commissionCandidats
                                                        ? $adherent->commissionCandidats->last()
                                                        : null;
                                                @endphp
                                                @if (!$last || ($last && $last->decision != null))
                                                    {{-- {{ $adherent->user->id == 138 ? dd($last) : '' }} --}}
                                                    <button type="button"
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('.open-modal-btn').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const focalPoint = this.getAttribute('data-focal-point');

                        document.getElementById('adherentIdInput').value = adherentId;

                        // Réinitialiser la sélection
                        const focalPointSelect = document.getElementById('focalPointSelect');
                        focalPointSelect.value = '';

                        // Si un point focal existe déjà, le sélectionner automatiquement
                        if (focalPoint && focalPoint.trim() !== '') {
                            const option = focalPointSelect.querySelector(
                                `option[value="${focalPoint}"]`);
                            if (option) {
                                focalPointSelect.value = focalPoint;
                            }
                        }
