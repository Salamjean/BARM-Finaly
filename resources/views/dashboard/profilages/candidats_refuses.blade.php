    ...
    <div class="container-fluid">
        ...
        <div class="bg-white p-4 rounded-3 shadow-none mb-4">
            <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    <i class="bx bx-user-x text-danger fs-3 me-3"></i>
                    <div>
                        <h5 class="mb-1">Candidats refusés par le BARM</h5>
                        <p class="text-muted mb-0">Liste des candidats qui ont été refusés lors de l'évaluation BARM</p>
                    </div>
                </div>
                            ...
                            @forelse ($candidatures as $index => $candidat)
                                <tr class="align-middle">
                                    ...
                                    <td>
                                        <div class="text-center">
                                            @php
                                                $refusedPartnership = $candidat->candidaturePartners->where('status', 'refused')->first();
                                            @endphp
                                            @if($refusedPartnership)
                                                <div class="fw-bold text-dark">{{ \Carbon\Carbon::parse($refusedPartnership->updated_at)->format('d/m/Y') }}</div>
                                                <div class="small text-muted">{{ \Carbon\Carbon::parse($refusedPartnership->updated_at)->diffForHumans() }}</div>
                                            @else
                                    ...
                                    <td>
                                        <div class="text-wrap" style="max-width: 200px;">
                                            @php
                                                $refusedPartnership = $candidat->candidaturePartners->where('status', 'refused')->first();
                                            @endphp
                                            @if($refusedPartnership && $refusedPartnership->rejection_reason)
                                                <small class="text-muted">{{ $refusedPartnership->rejection_reason }}</small>
                                            @else
