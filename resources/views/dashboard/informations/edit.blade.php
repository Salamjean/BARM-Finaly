        ...
        <div class="col-xl-10 mx-auto">
            ...
            <div class="card">
                ...
                <div class="card-body p-4">
                    <form id="informationForm" class="row g-3" method="POST" action="{{route('info.update', $infos->id)}}" enctype="multipart/form-data">
                        @csrf
                        @if (can('responsable-communication'))
                        <div class="col-md-6">
                            <label for="nom" class="form-label">Titre <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" placeholder="Titre de l'information" name="title" value="{{ old('title') ?? $infos->title }}"/>
                            @error('title')
                        ...
                        <div class="col-md-12 form-group">
                            <label for="contenu" class="form-label">Message <span class="text-danger">*</span></label>
                                <textarea type="text" rows="5" class="form-control @error('contenu') is-invalid @enderror" id="contenu" placeholder="Détails de l'information" name="contenu">{{ old('contenu') ?? $infos->contenu }}</textarea>
                            @error('contenu')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
